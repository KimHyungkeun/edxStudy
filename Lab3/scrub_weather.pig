...
Source = LOAD '/user/hkkim/data/weather' USING PigStorage('\t') AS (year:chararray, month:int, maxtemp:float, mintemp:float, frostdays:int, rainfall:float, sunshinehours:chararray);
...
STORE SortedReadings INTO '/user/hkkim/data/scrubbedweather' USING PigStorage(' ');